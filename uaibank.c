...
void search_id(int id, User users[], int users_qnty) {
  int found = 0;
  //criar struct pra amarzecar o nome idade ou saldo da pessoa . 

  for (int i = 0; i < users_qnty; i++) {
    if (users[i].id == id) {
      //criar vetor e colocar nome, idade e saldo atual da pessoa 
      found = 1;
      break;
    }
  }
  ...
  //dar return pra retornar esse vetor
}
...
void transfer_money(int id_origin, int id_destination, User users[],
  ...
  for (int i = 0; i < users_qnty; i++) {
    if (users[i].id == id_origin) {
      if ((users[i].current_balance - amount) < 0) {
        printf("Erro, a pessoa não tem saldo para essa operação.\n");
      }
  ...
  printf("Transfêrencia realizada com sucesso!\n");
}
