...
void search_id(int id, User users[], int users_qnty) {
  ...
  for (int i = 0; i < users_qnty; i++) {
    if (users[i].id == id) {
      printf("ID: %d\nNome: %s\nIdade: %d\nSaldo atual: %.2f\n", users[i].id,
             users[i].name, users[i].age, users[i].current_balance);
      found = 1;
      break;
    }
  }
...
void remove_user(int id, User users[],
                 int *users_qnty) { // peguei o ponteiro para a quantidades de
                                    // usuário já que quero modificar o original
  int found = 0;
  for (int i = 0; i < *users_qnty; i++) {
    if (users[i].id == id) {
      found = 1;
...
void transfer_money(int id_origin, int id_destination, User users[],
                    int users_qnty, float amount) {
  User *origin = NULL;
  ...
  if (origin == NULL || destination == NULL) {
    printf("Erro: Usuário de origem ou de destino não encontrado.\n");
    return;
  }
  origin->current_balance -= amount;
  destination->current_balance += amount;
  //não sei se vai precisar mostrar isso 
  printf("Usuário de origem (id: %d, saldo atual: %.2f)\n", origin->id, origin->current_balance);
  printf("Usuário de destino (id: %d, saldo atual: %.2f)\n", destination->id, destination->current_balance);
...
int main() {
  ...
  do {
    printf("Selecione uma opção: \n[1] - Criar um novo usuário\n[2] - Criar "
           "multiplos novos usuários\n[3] - Buscar um usuário por ID\n[4] - "
           "Deletar um usuário por ID\n[5] - Transferência de saldo entre "
    ...
    case 5: {
      ...
      float amount;
      printf("Digite o id de origem, id de destino e a quantia que deseja "
             "transferir:\n");
